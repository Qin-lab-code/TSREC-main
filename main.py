...
parser.add_argument('--dataset', default='Tmall', help='dataset name: retailrocket/diginetica/Tmall')
...
def main():
    ...
    model = trans_to_cuda(TSREC(adjacency=train_data.adjacency, n_node=n_node, n_users=train_data.length,
                                 interaction_matrix=train_data.interaction, lr=opt.lr, l2=opt.l2, beta=opt.beta,
                                 lam=opt.lam, eps=opt.eps, layers=opt.layer, emb_size=opt.embSize,
